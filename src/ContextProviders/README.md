# Context Providers
> Facilitating State Management and Sharing
...
### [GoogleContext](./GoogleContext.jsx)
Manages the loading of the Google Charts library, providing access to Google Charts functionalities throughout the application. It ensures the library is only loaded **once** and is accessible in any component that requires it.

### [MetadataContext](./MetadataContext.jsx)
Facilitates the fetching (only once) and sharing of metadata of the whole application, including:
  - [NOT IN USE] Comment counts from the Hyvor Talk API for the [Comment Section](/src/Components/CommentSection.jsx) (only in `/dashboard/nyuad` page)
  - The current page (`/`, `/login`, or `/dashboard`)
  - A list of all the charts' title in the `dashboard` page

### [PreferenceContext](./PreferenceContext.jsx)
Sets user preferences based on `localStorage` or user selected values and ensures that user preferences are stored and shared across the application:
  - `themePreference`
...
Manages authentication state of CITIESair, used in tandem with [Account](/src/Components/Account/) UI components, `user` contains these keys props:
  - `is_verified`: self-explanatory, will throw snackbar notification if the user has not been verified
  - `email` and `username`: keep track of the user identity to display on the [Header](/src//Components/Header/)

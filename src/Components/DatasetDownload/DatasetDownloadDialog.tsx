...
type SensorsList = Record<string, SensorInfo>;

interface PreviewingDataset {
  sensor: string;
...
export default function DatasetDownloadDialog({ onButtonClick }: DatasetDownloadDialogProps) {
  ...
  const [previewingDataset, setPreviewingDataset] = useState<PreviewingDataset | null>(null);

  // Construct the structure of sensorsList based on current data
  const sensorsList: SensorsList = useMemo(() => {
...
interface DatasetProps {
  ...
  setPreviewingDataset: (dataset: PreviewingDataset | null) => void;
  isPreviewing: boolean;
}

const Dataset = (props: DatasetProps) => {
  const { sensorsList, sensor, previewingDataset, setPreviewingDataset, isPreviewing } = props;
  const [aggregationType, setAggregationType] = useState<string>(AggregationType.hour);
...
const PreviewDataset = ({ previewingDataset, sensorsList }: PreviewDatasetProps) => {
  ...
  const [previewingDatasetName, setPreviewingDatasetName] = useState<string>(INIT_PREVIEWING_DATASET_NAME);
  const [csvFileName, setCsvFileName] = useState<string>(INIT_CSV_FILE_NAME);

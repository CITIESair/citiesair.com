...
interface AQImapProps {
    overridenThemePreference?: string | null;
    ...
    mapData: CurrentSensorsData | null | undefined;
...
const AQImap = (props: AQImapProps) => {
    ...
    const { center, fitBounds, maxBounds: computedMaxBounds } = useMemo(
        () => {
            const coords = sanitizedMapData.map(loc => loc.sensor.coordinates);
    ...
    const createClusterCustomIcon = (cluster: any) => {
        ...
        const values = cluster
            .getAllChildMarkers()
            .map((m: any) => m?.options?.options?.[DataTypeKeys.pm2_5])   // get raw value (string or number)
            .map((v: any) => (v != null ? Number(v) : NaN))              // convert to Number, null/undefined → NaN
            .filter(Number.isFinite);                             // keep only valid numbers
        const avgPM25 = values.length ? values.reduce((a: number, b: number) => a + b) / values.length : null;
        ...
        const allStatuses = cluster
            .getAllChildMarkers()
            .map((m: any) => m?.options?.options?.sensor_status)
        ...
        return getAQIDivIcon({
            ...
            location: {
                sensor: {} as any,
                current: { aqi: { category, categoryIndex, val: aqi } } as any
            }
        });
    };
            ...
            return (
                ...
                >
                    {disableInteraction === false && (
                        <StyledLeafletPopup>
                            {
                                (location.sensor as any)?.public_iqair_station_link ?
                                    <Stack direction="row" spacing={smallScreen ? 1 : 3}>
                                        <Link
                                            variant={smallScreen ? 'body2' : 'body1'}
                                            fontWeight={500}
                                            href={(location.sensor as any).public_iqair_station_link}
                                            target='_blank'
    ...
    return (
        <Box
            ...
            <MapContainer
                center={centerCoordinates || center || undefined}
                zoom={defaultZoom}
                minZoom={minZoom}
                maxZoom={maxZoom}
                bounds={isServerDown ? [[24.521723, 54.43135], [24.52609, 54.43779]] as any : undefined}
                maxBounds={maxBounds || computedMaxBounds || undefined}
                scrollWheelZoom={false}

...
import { CurrentAQIGridSize, CurrentAQIGridSizeType } from './CurrentAQIGridSize';
...
const returnDefaultZoom = (currentSchoolID: string): number => {
...
interface CurrentAQIMapWithGridProps {
...
const CurrentAQIMapWithGrid = (props: CurrentAQIMapWithGridProps) => {
    ...
    // (Leaflet + React Query's quirk)
    const [mapData, setMapData] = useState<CurrentSensorsData | null>(null);
    ...
    const groupedSensorsBySortingId = useMemo((): GroupedSensorData => {
        ...
        currentSensorsData.forEach(item => {
            ...
            let sortingId: number | null = rawId == null ? null : Math.floor(Number(rawId));
        ...
        const rows = Object.keys(sensorsWithSortingId)
            ...
            .map(key => sensorsWithSortingId[Number(key)]);
            ...
            <Grid item xs={12} sm={6}>
                ...
                >
                    <AQImap
                        ...
                        overridenThemePreference={isOnBannerPage ? ThemePreferences.light : themePreference}
                        centerCoordinates={
                            isNYUAD ?
